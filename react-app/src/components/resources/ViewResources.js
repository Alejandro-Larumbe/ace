...
export default function ViewResources({ resources, dispatch, setResources }) {
  ...
  const onDelete = (id) => {
    (async () => {
      const deleted = await dispatch(deleteResource(id))
      if (deleted === 'resource deleted' ) {
              ...
              if (resource.resourceTypeId === type) {
                return (
                  ...
                  >
                    ...
                    <IconButton style={{ marginRight: '0' }} onClick={() => onDelete(resource.id)} edge="end" aria-label="comments">
                      <DeleteIcon />
                    </IconButton>
